...
class CRNN(nn.module):
    def __init__(self, time_steps=81, n_mels=40,
        ...
        self.num_features = math.floor((n_mels - kernel_size[1]) / stride[1] + 1)

        self.dropout = nn.Dropout(dropout)
        self.gru = nn.GRU(input_size=num_features * conv_channels, hidden_size=gru_hidden,
                          num_layers=gru_layers, dropout=dropout, batch_first=True)

    def forward(self, inputs):
        # inputs: (batch_size, 1, time_steps, n_mels)
